...
export const ImageMedia: React.FC<MediaProps> = (props) => {
  ...
  if (!src && resource && typeof resource === 'object') {
    ...
    // Use Payload-provided URL when valid; fallback to Payload media API by filename.
    const rawFilename = typeof filename === 'string' ? filename.replace(/^\/+/, '') : null
    const mediaUrl =
      typeof url === 'string' && url.trim()
        ? url
        : rawFilename
          ? `/api/media/file/${encodeURIComponent(rawFilename)}`
          : null
  ...
  return (
    <picture className={cn(pictureClassName)}>
      <NextImage
        ...
        placeholder={imagePlaceholder === 'empty' ? 'empty' : 'blur'}
        {...(imagePlaceholder === 'empty' ? {} : { blurDataURL: placeholderBlur })}
        priority={priority}
